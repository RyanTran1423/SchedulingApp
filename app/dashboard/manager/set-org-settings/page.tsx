...
export default async function SetOrgSettingsPage() {
  ...
  const [roles, savedHours, savedRequirements] = await Promise.all([
    findOrganizationRoles(manager.organization_id),
    findOrganizationWeeklyHours(manager.organization_id),
    findOrganizationRoleRequirements(manager.organization_id),
  ]);

  const initialWorkingHours = savedHours.map((hours) => ({
    id: hours.id,
  ...
  const initialRoleRequirements = savedRequirements.map((requirement) => ({
    id: requirement.id,
    organizationRoleId: requirement.organization_role_id,
    dayOfWeek: requirement.day_of_week,
    startTime: formatTimeForInput(requirement.start_time),
    endTime: formatTimeForInput(requirement.end_time),
    ...
    <section>
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600">
          Configure your weekly working hours and staffing requirements.
        </p>
      </div>
      ...
      <OrganizationSettingsForm
        ...
        initialRoleRequirements={initialRoleRequirements}
      />
    </section>
  );
}